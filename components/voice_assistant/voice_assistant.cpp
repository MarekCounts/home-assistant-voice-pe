...
// URL resolution, connection setup, and decoder startup can legitimately exceed the old two-second playback
// timeout. Keep startup separately bounded so a missing/failed URL cannot leave the assistant stuck forever.
static const uint32_t MEDIA_PLAYER_START_TIMEOUT_MS = 15000;
static const char *const MEDIA_PLAYER_START_TIMEOUT = "await-media-player-start";
static const char *const MEDIA_PLAYER_PLAYBACK_TIMEOUT = "media-player-playing";
#endif
    ...
    this->media_player_->add_on_state_callback([this](media_player::MediaPlayerState state) {
      switch (state) {
        case media_player::MediaPlayerState::MEDIA_PLAYER_STATE_ANNOUNCING:
          if (this->media_player_response_active_ &&
              this->media_player_response_state_ == MediaPlayerResponseState::URL_SENT) {
            // Playback really started. From here on the normal playback watchdog, not the startup gate,
            // controls completion.
            this->cancel_timeout(MEDIA_PLAYER_START_TIMEOUT);
            ...
            ESP_LOGD(TAG, "Media player announcement started (generation %" PRIu32 ")",
                     this->media_player_response_generation_);
          }
          break;
        default:
          if (this->media_player_response_active_ &&
...
#ifdef USE_MEDIA_PLAYER
    this->cancel_media_player_response_(true);
...
#ifdef USE_MEDIA_PLAYER
      if (this->media_player_ != nullptr) {
        // A new pipeline invalidates every response callback from the prior run. Do not clear
        // continue_conversation_ here: START_PIPELINE still needs it to decide whether to skip the wake-word stage.
        this->cancel_media_player_response_(true);
        this->started_streaming_tts_ = false;
      }
#endif
    ...
    case State::STREAMING_RESPONSE: {
      ...
      if (this->media_player_ != nullptr) {
        ...
        if (this->media_player_response_state_ == MediaPlayerResponseState::FINISHED) {
          const uint32_t generation = this->media_player_response_generation_;
          this->finish_media_player_response_(generation, true);
          break;
        }
      }
#endif
...
#ifdef USE_MEDIA_PLAYER
      if (this->media_player_ != nullptr && this->media_player_response_active_) {
        // A speaker-path event or stale state transition must not bypass the media-player lifecycle gate.
        break;
      }
#endif
#ifdef USE_SPEAKER
...
void VoiceAssistant::request_stop() {
#ifdef USE_MEDIA_PLAYER
  const bool had_media_player_response = this->media_player_response_active_;
  const bool had_streaming_tts = this->started_streaming_tts_;
  this->cancel_media_player_response_(true);
  this->started_streaming_tts_ = false;
#endif
  this->continuous_ = false;
  ...
  switch (this->state_) {
    ...
    case State::STREAMING_RESPONSE:
#ifdef USE_MEDIA_PLAYER
      if (had_streaming_tts) {
        // Haven't reached the TTS_END stage, so send the stop signal to HA.
        this->signal_stop_();
      }
      if (had_media_player_response) {
        // Cancellation invalidates the media-player callback, so finish the local state transition explicitly.
        this->set_state_(State::IDLE, State::IDLE);
      }
#endif
      break;
...
void VoiceAssistant::start_playback_timeout_() {
  ...
  if (this->media_player_ != nullptr && this->media_player_response_active_) {
    const uint32_t generation = this->media_player_response_generation_;
    this->set_timeout(MEDIA_PLAYER_PLAYBACK_TIMEOUT, 2000, [this, generation]() {
      if (!this->media_player_response_active_ || generation != this->media_player_response_generation_ ||
          this->media_player_response_state_ != MediaPlayerResponseState::PLAYING) {
        return;
      }

      // A delayed main loop must not turn a still-playing announcement into a successful completion. If the
      // player still reports ANNOUNCING, retain the playback gate and give its state callback another interval.
      if (this->media_player_->state == media_player::MEDIA_PLAYER_STATE_ANNOUNCING) {
        this->start_playback_timeout_();
        return;
      }

      ESP_LOGW(TAG, "Media player stopped announcing without a state callback (generation %" PRIu32 ")", generation);
      this->finish_media_player_response_(generation, true);
    });
    return;
  }
...
#ifdef USE_MEDIA_PLAYER
void VoiceAssistant::begin_media_player_response_() {
  // Stop and invalidate an older response before assigning the next generation. Media-player control calls are
  // processed in order, so the subsequent play call cannot be mistaken for the response that was just cancelled.
  this->cancel_media_player_response_(true);

  this->media_player_response_generation_++;
  if (this->media_player_response_generation_ == 0) {
    // Reserve zero for startup diagnostics and keep generation comparisons unambiguous after wraparound.
    this->media_player_response_generation_++;
  }
  const uint32_t generation = this->media_player_response_generation_;

  this->media_player_response_active_ = true;
  this->media_player_response_started_ = false;
  this->media_player_response_state_ = MediaPlayerResponseState::URL_SENT;

  // Arm the start gate before dispatching the URL. A very fast media player may publish ANNOUNCING from the
  // control call itself; its callback will cancel this timeout and enter the playback phase.
  this->set_timeout(MEDIA_PLAYER_START_TIMEOUT, MEDIA_PLAYER_START_TIMEOUT_MS, [this, generation]() {
    if (!this->media_player_response_active_ || generation != this->media_player_response_generation_ ||
        this->media_player_response_state_ != MediaPlayerResponseState::URL_SENT) {
      return;
    }

    ESP_LOGW(TAG, "Timed out waiting for media player announcement to start (generation %" PRIu32 ")", generation);
    if (this->media_player_ != nullptr) {
    ...
    // A response that never became audible must fail closed. In particular it must not turn the next pipeline
    // into a wake-word-free continuation.
    this->continue_conversation_ = false;
    this->finish_media_player_response_(generation, false);
  });

  ESP_LOGD(TAG, "Waiting for media player announcement (generation %" PRIu32 ")", generation);
}

void VoiceAssistant::finish_media_player_response_(uint32_t generation, bool success) {
  if (!this->media_player_response_active_ || generation != this->media_player_response_generation_) {
    return;
  }

  // Never report a successful response that did not pass through ANNOUNCING. This is the central continuation
  // gate: only a response known to have started can eventually reopen the microphone.
  if (success && !this->media_player_response_started_) {
    ESP_LOGW(TAG, "Rejecting completion for a media player response that never started (generation %" PRIu32 ")",
             generation);
    success = false;
  }

  this->cancel_timeout(MEDIA_PLAYER_START_TIMEOUT);
  this->cancel_timeout(MEDIA_PLAYER_PLAYBACK_TIMEOUT);

  // Mark inactive before sending API messages or changing component state so re-entrant callbacks cannot emit a
  // duplicate completion.
  this->media_player_response_active_ = false;
  this->media_player_response_started_ = false;
  ...
  ESP_LOGD(TAG, "Media player announcement finished (generation %" PRIu32 ", success=%s)", generation, YESNO(success));
  if (this->api_client_ != nullptr) {
    api::VoiceAssistantAnnounceFinished msg;
    msg.success = success;
    this->api_client_->send_message(msg);
  ...
  if (this->mic_source_->is_running() || (this->mic_source2_ != nullptr && this->mic_source2_->is_running())) {
    this->set_state_(State::STOP_MICROPHONE, State::RESPONSE_FINISHED);
  } else {
    this->set_state_(State::RESPONSE_FINISHED, State::RESPONSE_FINISHED);
  }
}

void VoiceAssistant::cancel_media_player_response_(bool stop_player) {
  this->cancel_timeout(MEDIA_PLAYER_START_TIMEOUT);
  this->cancel_timeout(MEDIA_PLAYER_PLAYBACK_TIMEOUT);

  const bool was_active = this->media_player_response_active_;
  if (was_active && stop_player && this->media_player_ != nullptr) {
    this->media_player_->make_call()
        .set_command(media_player::MEDIA_PLAYER_COMMAND_STOP)
        .set_announcement(true)
        .perform();
  ...
  if (was_active) {
    // Invalidate callbacks already removed from the scheduler heap but queued for execution in this loop pass.
    this->media_player_response_generation_++;
  }
}
#endif
      ...
      if (this->media_player_ != nullptr) {
        for (const auto &arg : msg.data) {
          if ((arg.name == "tts_start_streaming") && (arg.value == "1") && !this->tts_response_url_.empty()) {
            const std::string response_url = this->tts_response_url_;
            this->begin_media_player_response_();
            this->set_state_(State::STREAMING_RESPONSE, State::STREAMING_RESPONSE);

            this->media_player_->make_call().set_media_url(response_url).set_announcement(true).perform();

            this->started_streaming_tts_ = true;

            tts_url_for_trigger = response_url;
