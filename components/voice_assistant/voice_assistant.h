...
class VoiceAssistant final : public Component {
 ...
 protected:
...
#ifdef USE_MEDIA_PLAYER
  // Begin a new media-player response and arm its bounded URL_SENT -> ANNOUNCING gate.
  // This must run before dispatching the media-player call.
  void begin_media_player_response_();
  // Complete the current response exactly once. The generation rejects callbacks belonging to an older run.
  void finish_media_player_response_(uint32_t generation, bool success);
  // Cancel all response timers and optionally stop the active announcement without reporting completion.
  void cancel_media_player_response_(bool stop_player);
#endif
  ...
  uint32_t media_player_response_generation_{0};
  bool media_player_response_active_{false};
